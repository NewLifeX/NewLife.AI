...
/// <summary>对话执行管道上下文。携带单次请求所需的用户与会话上下文，在 PrepareContext/StreamAsync/CompleteAsync 之间流转</summary>
public class ChatPipelineContext : IExtend
{
    ...
    /// <summary>实际使用的最大 Token 数。由管道在构建 ChatOptions 后填充</summary>
    ...
    /// <summary>请求级扩展参数。最终通过 ChatOptions.Items 注入服务商。
    /// 支持服务商专属参数，如 DashScope 的 EnableSearch / SearchStrategy / ThinkingBudget / TopK 等</summary>
...
/// <summary>对话执行管道。封装能力扩展层（工具调用、技能注入）与知识进化层（记忆注入、自学习、事件智能体），对外提供统一执行接口</summary>
/// <remarks>
/// 典型实现在 DI 装配时组装好三层能力，内核层通过本接口驱动执行，无需感知各层细节。
/// <code>
