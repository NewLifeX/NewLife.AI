...
本文档采用**四层分层架构**（L1 接入 → L2 内核 → L3 能力 → L4 知识）叙述 NewLife.AI 社区版的完整设计。
模块编码：**AI**（AI Client SDK）、**CHAT**（对话系统）、**TOOL**（能力扩展）、**EVO**（知识进化基础版）、**GW**（API 网关）、**IM**（渠道集成）、**AGENT**（智能体框架）。
...
├──────────────────────────────────────────────────────────┤
│  L3 能力扩展层                                             │
│    技能(TOOL) / 原生工具(TOOL) / MCP(TOOL)               │
│    / IChatFilter(TOOL) / IChatHandler(TOOL)              │
...
> ⚠️ 社区版不包含 KNOW（知识库 RAG）、PROJ（项目运营）、DESK（桌面客户端）模块及 TOOL/GW/EVO/IM 各模块的高级子功能。完整功能范围见[需求文档](需求文档.md)和[功能清单](功能清单.md)。
...
| 文档 | `build_ppt` | `BuildPptToolService` | PowerPoint 演示文稿生成，支持 widgetSrc 模式嵌入 show_widget 卡片 |
| 文档 | `build_excel` | `BuildExcelToolService` | Excel 电子表格生成 |
| 文档 | `build_doc` | `BuildDocToolService` | Word 文档生成 |
...
NewLife.AI 社区版提供**基础级**知识进化能力，让 AI 自动从对话中提取结构化用户记忆。
...
| TOOL | 数据库工具：search_table + query_sql | ✅ | |
