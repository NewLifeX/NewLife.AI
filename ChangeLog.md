...
## v1.4.2026.0706 (2026-07-06)

### 编程智能体
- **CodingAgent**：新增编程智能体，支持分析/只读与代码修改任务分流执行，含完整工具集与管道优化
- **工具调用事件链路**：新增工具调用事件链路，支持 LLM 摘要字段传递与工具执行回调
- **CancellationToken 全链路**：完善异步取消链路，提升任务中断响应速度

### 全局系统指令
- **全局系统指令配置**：支持全局系统指令注入所有对话，统一行为约束
- **用户隔离配置**：控制 User 字段透传到 LLM，支持多租户场景

### API 网关增强
- **网关工具执行**：支持按 AppKey 配置服务端工具执行与工具清单管控
- **自动工具执行模式**：网关支持自动工具执行与透传模式切换

### TTS 语音合成
- **DashScope TTS 全系列**：支持千问 TTS 全系列语音合成与 48 音色选择
- **CosyVoice V3.5**：能力区分与参数对齐官方，支持流式合成
- **端到端流式合成**：TTS 支持端到端流式合成与流式播放，前端实时进度展示
- **多格式支持**：支持 Opus 音频格式端到端归档与选择，音量/音调/语速全链路调节

### 模型增强
- **四档定价体系**：AI 模型支持四档价格，前后端全量适配，定价参数推断完善
- **模型锁定保护**：支持模型显式加锁/解锁，自动能力保护机制
- **推理强度 ReasoningEffort**：支持模型推理强度全链路能力
- **DeepSeek 余额查询**：支持 DeepSeek 余额查询及服务注册扩展
- **模型能力字段细化**：拆分音频能力、新增重排序等，完善 TTS/重排序能力识别

### 可视化卡片与文档生成
- **六大可视化卡片**：支持图表、SVG/HTML、看板、时间轴、脑图、地图标注等卡片展示
- **Office 文档生成**：支持生成 Excel/Word/PPT 文档
- **Shiki 代码高亮**：切换 Shiki 实现代码高亮，优化 Markdown 渲染与错误提示

### 工具调用增强
- **工具调用去重**：支持同轮与跨轮次去重，前后端双端防重复渲染
- **结构化工具结果**：统一返回 ToolResult，支持多受众内容分发与前后端分离错误提示
- **工具调用 Token 限额**：引入工具调用 Token 总限额配置及前后端支持
- **工具上下文重构**：重构工具上下文处理器，前端支持多类型可视化

### 前端重构
- **UI 体系大升级**：组件/主题全面重构，品牌主色切换为翠绿色
- **技能栏组件**：新增技能栏/弹窗/下拉组件，统一类型与国际化
- **会话卡片风格**：支持会话卡片风格选择，前后端全链路适配
- **文件上传校验**：前端支持文件类型白名单校验与动态配置
- **Material Icons 本地化**：本地化 Material Icons/Symbols 字体依赖

### 意图门控与技能
- **意图门控**：新增意图门控功能及主技能支持，优化前后端交互
- **技能触发词**：支持技能触发词配置与会话 SkillId 粘滞优化
- **技能激活重构**：技能激活与门控链路重构，消息技能追溯增强

### 用户记忆与知识
- **记忆确认功能**：支持用户核实 AI 推断记忆，提升记忆准确性
...
### 其他优化
- 思考过程默认折叠，提升阅读体验
- 欢迎副标题与错误引导文案配置化，支持前后端动态渲染
- 客服入口支持"隐藏"选项，支持多位置自定义展示
- 支持后台生成，提升会话切换与断线体验
- 支持匿名访问分享，分享链接优化
- MCP 工具支持禁用配置，完善通配与文档说明
- Ollama 流式 tool_calls 终结帧修复，默认忽略 null 字段
- DbQueryToolService 表结构查询分组输出及映射优化
