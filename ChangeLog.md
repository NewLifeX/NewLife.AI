...
## v1.6.2026.0902 (2026-09-02)

### MCP 协议
- **MCP 全链路增强**：协议核心自包含重构，移除 Remoting 依赖，服务端/客户端/传输/测试/文档全链路对标官方 SDK
- **非 Web 兼容**：增强 MCP 服务端非 Web 场景兼容性，完善文档与测试
- **EnableMcp 配置**：新增 EnableMcp 配置及工具调用短路逻辑

### 协议客户端
- **多协议客户端 v5.0 增强**：支持 Anthropic 多轮思考回传（签名/redacted_thinking/adaptive/effort/采样剥离/budget clamp）、Ollama/DashScope 多轮思考回放、ToolChatClient 签名透传
- **容错与重试**：新增 SDK 重试机制，参数映射、用量明细、finish_reason 透传等协议级一致性全面修复
- **Ollama 协议支持**：网关兼容 Ollama 协议，配套集成测试闭环

### 工具调用与安全
- **工具链安全加固**：SSRF 防护与状态隔离，工具调用去重与熔断隔离粒度提升
- **工具服务参数类型化**：工具服务参数类型化与 JSON 解析增强，支持大体积 SVG/HTML/图表结果截断
- **ToolRegistry 优化**：优化工具注册反射与错误处理，注册冲突可观测性提升

### 模型与元数据
- **模型元数据表**：模型报价单扩展为元数据表，历史型号统一管理，覆盖主流服务商
- **家族化能力推断**：模型能力推断家族化，支持 qwen3.7 系列 1M 上下文识别
- **统一 Token 估算**：统一 Token 估算逻辑至 TokenEstimator，统一 128K 上下文窗口

### 配置与领域模式
- **强类型配置枚举**：配置项重构为强类型枚举，提升类型安全
- **渠道领域模式**：合并 EnableChannelFullHandlers/EnableChannelPersist 为 EnableChannelDomainMode，统一控制渠道领域模式
- **用户隔离**：新增"启用安全拦截反哺""用户隔离"配置项，支持 Guardrails 痛觉记忆与 LLM User 隔离

### 会话与记忆
- **统一向量存储接口**：语义记忆重构为统一向量存储接口，支持注入式缓存后端
- **MemoryCacheSessionStore**：新增会话内存缓存存储及单元测试
- **滑动窗口上下文**：移除会话轮数硬限制，启用滑动窗口上下文

### 搜索与知识
- **新增搜索通道**：新增 Bing RSS/搜狗/DuckDuckGo 搜索通道
- **知识库门控**：知识库支持公开范围门控与关键字搜索

### 流式与对话
- **SSE 控制器基类拆分**：新增轻量对话服务及 SSE 控制器基类拆分
- **SSE 稳定性**：修复 SSE 流中断异常，非流式对话补全 finish_reason 透传

### 性能优化
- **HashTextEmbedder 优化**：混合去重哈希优化，3 倍提速、14 倍降分配，零分配优化
- **HttpClientPool 连接池**：引入静态池化复用连接，优化 Handler 生命周期与复用管理
- **上下文守卫**：上下文窗口超限识别与守卫优化，异常类型化

### 基准测试
- **Benchmark 项目**：新增基准测试项目，覆盖请求构建、嵌入、Schema 构建、工具循环等关键路径
- **工具循环基准**：扩展含真实工具调用的工具循环与流式 SSE chunk 解析基准

### 前端
- **推理过程升级**：推理过程展示升级为枚举布局，支持右侧分栏与历史推理导航
- **系统设置补齐**：系统设置前端配置补齐，新增错误引导/重排序模型/MCP工具/SQL操作四项配置

### Bug 修复
- **[fix]** 修复 chart data series 字段为对象时崩溃问题
- **[fix]** 修复 HttpClientPool 长流释放问题
- **[fix]** 修复 Anthropic/Gemini 工具定义丢失及多模态输入跨协议一致性问题
- **[fix]** 修复 SSE 流中断与上下文窗口超限识别问题

---

## v1.5.2026.0802 (2026-08-02)

### 新渠道与模型路由
- **新增 5 个 IM 渠道**：新增 WeComBot/微信公众平台/微信客服/QQ/Discord 渠道，渠道体系扩展至 11 个
- **主备模型路由**：支持主备模型路由与 Provider 屏蔽机制，提升模型服务可用性
- **模型设置字段扩展**：新增模型设置字段及嵌入参数定制，支持批量初始化模型

### 技能体系
- **技能项目维度**：技能接口与服务支持项目维度及专属能力配置
- **技能归属管理**：区分并管理技能全局与个人归属，权限更清晰

### 工具调用增强
- **工具别名与动态表**：增强工具别名、回复兜底与动态表支持
- **数据库工具统一**：统一数据库工具接口为 run_sql，SQL 参数支持 query/sql 别名兼容
- **表格模型升级**：表格模型 Rows 字段升级为二维数组，适配复杂数据结构
- **JSON 解析统一**：统一 JSON 解析逻辑，增强数组参数兼容性
- **文档生成增强**：扩展结构化数据模型，文档生成工具能力提升
- **工具结果安全截断**：优化 ToolChatClient 参数与内容截断逻辑，防止破坏代理对

### 安全与消息流
- **守密指令注入**：注入守密指令并检测 AI 回复泄密特征，提升对话安全
- **SSE 序列化防御**：增强 SSE 事件序列化防御，截断超长字段

### 前端与国际化
- **国际化适配**：大规模国际化适配，引入 useTranslation 与多语言资源
- **Wiki Tab 集成**：前端集成 Wiki Tab 与后端服务注册优化
- **旧浏览器兼容**：支持 legacy 构建与 Tailwind v4 降级方案，兼容旧版移动浏览器
- **前端性能优化**：依赖瘦身，Mermaid/多语言/弹窗懒加载优化

### 反馈与交互
- **多维评分反馈**：升级点赞/点踩为多维度评分反馈机制
- **知识领域消歧**：知识文章支持领域消歧，工具链路失败处理增强

### 健壮性与稳定性
- **Cron 重构**：移除 Cronos 依赖，重构 Cron 校验与表达式转换
- **批量嵌入优化**：调整批量嵌入默认批次为 10 并增强异常处理
- **任务调度增强**：提升任务调度与流式解析的健壮性
- **SSE 心跳优化**：优化 SSE 心跳与事件流收集，增强异常处理

### 其他优化
- **默认模型升级**：默认大模型升级至 qwen3.6-flash
- **依赖升级**：升级 NewLife 组件及适配新版 API 变更
- **表结构搜索优化**：优化表名搜索排序逻辑，新增匹配层级区分
- **归档文件命名**：归档文件命名优化，移除 ai-类型- 前缀
- **可视化工具扩展**：可视化与交互工具体系扩展，完善文档结构

### Bug 修复
- **[fix]** 修复 Mermaid 甘特图跨午夜渲染失败的问题
- **[fix]** 跳过 2 个 DashScope 集成测试（API URL 配置错误）
