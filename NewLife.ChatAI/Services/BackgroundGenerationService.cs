...
public class BackgroundGenerationService
{
    ...
    #region 构造
    /// <summary>实例化后台生成服务</summary>
    /// <param name="log">日志</param>
    public BackgroundGenerationService(ILog log) => _log = log;
    #endregion
    ...
    private async Task ConsumeAsync(BackgroundTask task, IAsyncEnumerable<ChatStreamEvent> eventStream, Func<BackgroundTask, Task>? onComplete, CancellationToken cancellationToken)
    {
        try
        {
            ...
            _log?.Info("后台生成任务完成，消息 {0}，内容长度 {1}", task.MessageId, task.ContentBuilder.Length);
        }
        ...
        catch (Exception ex)
        {
            ...
            _log?.Error("后台生成任务失败，消息 {0}: {1}", task.MessageId, ex.Message);
        }
        finally
        {
            ...
            if (onComplete != null)
            {
                ...
                {
                    _log?.Error("后台生成回调失败: {0}", ex.Message);
                }
            }
        }
    }
