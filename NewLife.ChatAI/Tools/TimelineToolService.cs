...
public class TimelineToolService(ILog log)
{
    ...
    /// <param name="items">
    /// 时间轴事件 JSON 数组。每项字段：
    /// - date（必填）：任意格式，如 "2024-01"、"2024年Q3"、"第一阶段"
    /// - title（必填）：事件标题（≤ 30 字）
    /// - description（可选）：补充说明（≤ 80 字）
    /// - color（可选）：十六进制颜色，如 "#5470c6"；不填则自动轮转配色
    /// - category（可选）：事件类别标签，如 "里程碑"、"发布"、"问题"
    /// 示例：[{"date":"2024-01","title":"v1.0 发布","description":"基础功能上线","color":"#5470c6","category":"里程碑"}]
    /// </param>
    ...
    /// <param name="palette">全局配色方案 JSON 数组</param>
    ...
    public ToolResult ShowTimeline(
        ...
        [Description(@"时间轴事件 JSON 数组。示例：[{""date"":""2024-01"",""title"":""v1.0发布"",""description"":""基础功能上线"",""color"":""#5470c6"",""category"":""里程碑""}]。description/color/category 可省略")] String items,
        ...
        [Description("全局配色方案 JSON 数组，如 [\"#3b82f6\",\"#10b981\",\"#f59e0b\"]。覆盖默认 8 色轮转。各 item.color 优先级更高")] String? palette = null,
        ...
        ToolCallContext? context = null)
    {
        ...
        try
        {
            itemsNode = JsonNode.Parse(items)
                ?? throw new ArgumentException("items 解析后为 null", nameof(items));
        }
        ...
        if (!palette.IsNullOrEmpty())
        {
            try
            {
                result["palette"] = JsonNode.Parse(palette);
            }
            catch (Exception ex)
            {
                log.Warn("[Timeline] palette 解析失败，已忽略：{0}", ex.Message);
            }
        }
        ...
        return new ToolResult(
            ...
            ToolContent.ForLlm($"[已渲染时间轴到客户端：{title}]"));
    }
