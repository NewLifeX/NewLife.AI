...
public class BuildPptToolService(ILog log)
{
    ...
    public async Task<ToolResult> BuildPpt(
        ...
        [Description(@"幻灯片数组（JSON）。1个元素=单页；多个=完整PPT。与widgetSrc互斥，二选一。每页字段：title、layout(title_only/title_content/two_column/chart_only/blank)、background(背景色16进制)、footer(页脚)、showPageNumber(显示页码true/false)、elements(元素列表)、notes(备注)、transition(切换)。elements支持：text(role=title/subtitle/body/kpi/caption，可加alignment=l/ctr/r、backgroundColor、color、runs富文本数组)、table(headers+rows，可加tableStyle={headerBgColor/headerFontColor/stripeColor}美化)、chart(chartType=bar/line/pie/area/scatter+categories+series)、image(src)、shape(shape={shapeType=rect/ellipse/roundRect/triangle/diamond,fillColor,lineColor,text,fontColor})。示例：[{""title"":""封面"",""layout"":""title_only"",""background"":""0F172A"",""elements"":[{""type"":""text"",""role"":""title"",""content"":""Q2汇报"",""alignment"":""ctr"",""color"":""60A5FA""},{""type"":""text"",""role"":""subtitle"",""content"":""新生命团队"",""color"":""94A3B8""}]}]")] String? slides = null,
